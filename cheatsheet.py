...
"""
# also %pdoc %pdef %psource  for docstring, function definition, source
# code only.
...
"""
%matplotlib  # set matplotlib to work interactively; does not import anything
%matplotlib inline
...
"""
ipython.exe
qtconsole - -pylab = inline - -ConsoleWidget.font_size = 10
"""
...
# In Python, everything is an object. A variable is like a name tag attached to an object, not a box that stores a value.
# Assignment statements modify namespaces, not objects.
# In other words,

name = 10
# means that you’re adding the name “name” to your local namespace, and
# making it refer to an integer object containing the value 10.
...
# vars(x) returns x.__dict__ the dictionary where Python objects store
...
# considered True: True, non-zero numbers, non-empty strings / sequences,
# any object not explicitly False
...
# Three ways to declare a string in Python: single quotes ('), double
...
# in Python, a string is just a sequence, so we can iterate over it!
for c in string:
    print(c)
...
# beware: when building a new list by multiplying, Python copies each item
# by reference
zeros = [0] * 5
...
# multiple assignment; outer tuple created from items separated by commas
...
# field names separated by space or comma
person = collections.namedtuple("person", "name firstName age")
...
# Although tuples are normally immutable, one can change field values via
# _replace():
...
for value in dict2.values():  # Iterate via values
    print(value)
# Dictionary comprehension in Python 2.7 or later
dict5 = {x: dict2[x] + 1 for x in dict2}
...
navs = {}
for (portfolio, equity, position) in data:
    # set if necessary, then return default
...
# always have to be written inside parentheses
...
# that where listcomps are greedy, generator expressions are lazy [use
# iterators]. Listcomps compute the entire result list all at once, as a
# list. Generator expressions compute one value at a time, when needed, as
# individual values. This is especially useful for long sequences where
# the computed list is just an intermediate step and not the final result.
...
# The default value of a function argument is evaluated at the point of
...
def make_dictionary(max_length=10, **entries):
    return dict(
        [(key, entries[key]) for i, key in enumerate(entries.keys()) if i < max_length]
    )
...
# string or tuple, the result will be of the same type; otherwise, it is
# always a list.
...
try:
    ...
    TypeError,
    NameError,
):  # multiple exceptions. parentheses are required
    pass
...
def g(x):
    if x < 0:
        raise ValueError("g(x) - negative number not allowed")
...
for func_name, value in inspect.getmembers(_type, predicate=inspect.ismethod):
    ...
    print(inspect.getfullargspec(value))
    print(value.__doc__)
...
for name, value in inspect.getmembers(inst, predicate=inspect.ismethod):
    ...
    if not name.startswith("_"):
        ...
        )  # Returns a named tuple ArgSpec(args, varargs, keywords, defaults).
...
def namespaced_function(function, global_dict, defaults=None):
    ...
    new_namespaced_function = types.FunctionType(
        function.__code__, global_dict, name=function.__name__, argdefs=defaults
    )
...
x = np.float32(
    1.0
)  # convert to array scalar: type(x) = numpy.float32, x.dtype = dtype('float32')
y = np.int_(
    [1, 2, 4]
)  # convert to array([1, 2, 4]): type(y) = numpy.ndarray, y.dtype = int32
...
np.linspace(
    1.0, 4.0, 6
)  # arrays with a specified number of elements, and spaced equally between the specified beginning and end values.
np.indices(
    (3, 3)
)  # set of arrays (stacked as a one-higher dimensioned array), one per dimension with each representing variation in that dimension.
...
z[
    [1, 1, 1, 1]
]  # == z[ np.array([1,1,1,1]) ] - produces an array of dimension n-1 since only one array index is given
...
x = np.zeros(
    3, dtype="3int8, float32, (2,3)float64"
)  # array of 3 records containing a 1D array of 3 int8, a float and 2x3 flot64 matrix.
x
...
a = exp(-((X - 2.5) ** 2 + (Y - 4) ** 2) / 4) - exp(
    -((X - 7.5) ** 2 + (Y - 4) ** 2) / 4
)  # shape of X, Y, a: (41, 51)
c = plt.contour(x, y, a)
...
fig, ax = subplots()
p = ax.pcolor(
    X / (2 * pi), Y / (2 * pi), Z, cmap=cm.RdBu, vmin=abs(Z).min(), vmax=abs(Z).max()
)
cb = fig.colorbar(p)
...
fig, ax = subplots()
cnt = contour(
    Z, cmap=cm.RdBu, vmin=abs(Z).min(), vmax=abs(Z).max(), extent=[0, 1, 0, 1]
)
...
ra = N.array(
    [(3.14159, "some string", [1, 2, 3, 4, 5], N.arange(0, 10, 0.1))],
