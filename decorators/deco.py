...
def line_arg2(must_have_argument=True, msg=None):
    ...
    def dec(func, *args, **kw):
        itr = [
            x for x in itertools.chain(args, list(kw.values())) if x != ""
        ]  # ignore any empty string
        # skip first argument (i.e. "self")
        argument_present = bool([arg for arg in itr][1:])
...
# self._default_ext = default_ext # default extension of the file, if not
# provided
