...
class ParseOptions(object):
    ...
    def __init__(
        self, has_argument, msg_arg, file_existence, msg_existence, default_ext
    ):
        super(ParseOptions, self).__init__()
        self.has_argument = (
            has_argument
        )  # True if a non-empty 'line' argument must be provided."""
        self.msg_arg = (
            msg_arg
        )  # The message to display if the "line" argument is empty("") but should not (or vice versa).
        # True if the argument is a file path and the file must exist, False if
        # it must not exist, None if we don't care / not a file path
        self.file_existence = file_existence
        self.default_ext = default_ext  # default extension of the file, if not provided

        # Better require a non-empty argument before checking its existence as
        # a file path.
        ...
        if self.msg_arg is None:
            if self.file_existence is None:
                self.msg_arg = (
                    ...
                    else "This command takes no argument."
                )
            else:
                self.msg_arg = "No file name or path was specified."

        default_msg_existence = (
            "The file {0} does not exist."
            if self.file_existence
            else "The file {0} already exists."
        )
        self.msg_existence = (
            default_msg_existence if msg_existence is None else msg_existence
        )
...
class parsecmd(object):
    ...
    def __init__(
        ...
        self._options = ParseOptions(
            has_argument, msg_arg, file_existence, msg_existence, default_ext
        )
    ...
    def __call__(self, func):
        ...
        def wrapper(self2, line):
            try:
                ...
                # argspec is a namedtuple ArgSpec(args, varargs, keywords,
                # defaults)
                if len(argspec.args) != len(args) + 1:
                    raise SyntaxError(
                        "Wrong number or wrong type of arguments in the command line.\nExpected {0}. Got: self, {1}".format(
                            argspec.args, args
                        )
                    )
                ...
                # bound to the argument names, as if the function was called
                # with them.
