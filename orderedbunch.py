...
# NOTE: The big change in Python 3.7 is that the dictionaries are now ordered by default.
...
class OrderedBunch:  # must be an old-style class !!
    ...
    # called instead of normal mechanism (i.e. store the value in the instance
    # dictionary)
    def __setattr__(self, name, value):
        # print "<__setattr__: {0} {1}>".format(name, value)
        # do not simply execute self.name = value - this would cause a
        # recursive call.
        if name.startswith("_"):
            # "internal" attribute - store in __dict__ as usual
    ...
    def __repr__(self):
        tuplelist = list(self.items())
        optional = (
            ", displayorder={}".format(self._displayorder)
        ...
        # then you can modify representation with repr.aRepr./someattribute/ as
        # needed

    def __str__(self):
        if self._displayorder is not None:
            tuplelist = sorted(
                [(k, v) for k, v in list(self.items()) if k in self._displayorder],
                key=lambda e: self._displayorder.index(e[0]),
            )
        else:
            tuplelist = iter(list(self.items()))
    ...
    def copy_in_order(self, *displayorder):
        ...
        """
        defaultdisplayorder = (
            self._displayorder if self._displayorder is not None else list(self.keys())
        )
        todisplay = displayorder if len(displayorder) > 0 else defaultdisplayorder
        itr = ((k, v) for k, v in list(self.items()) if k in todisplay)
